...
export function TaskArchiveView({ task, posts, pagination, category, basePath }: { task: TaskKey; posts: SitePost[]; pagination: SiteFeedPagination; category: string; basePath: string }) {
  ...
  return (
    <EditableSiteShell>
      <main style={archiveVars} className="bg-[#FFF0E4] text-[var(--archive-text)]">
        {task === 'article' ? (
          <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
            <div className="grid gap-5 lg:grid-cols-[1fr_320px] lg:items-start">
              <div className="overflow-hidden rounded-[1.1rem] border border-[var(--editable-border)] bg-white shadow-[0_10px_24px_rgba(0,121,121,0.12)]">
                <div className="border-b border-[var(--editable-border)] bg-[#24B1B1]/10 px-6 py-4">
                  <div className="inline-flex items-center gap-2 rounded-full border border-[#007979]/30 bg-white px-4 py-2 text-xs font-black uppercase tracking-[0.2em] text-[#007979]">
                    <Icon className="h-4 w-4" /> {label}
                  </div>
                </div>
                <div className="p-6 sm:p-8">
                  <h1 className="font-display text-4xl font-bold leading-[1.03] tracking-[-0.03em] text-[#007979] sm:text-5xl">
                    {voice?.headline || `Browse ${label}`}
                  </h1>
                  <p className="mt-4 max-w-2xl text-base leading-8 text-[#2d6e6e]">
                    {voice?.description || SITE_CONFIG.description}
                  </p>
                  <div className="mt-6 grid gap-3 sm:grid-cols-[1fr_auto] sm:items-stretch">
                    <div className="rounded-[0.9rem] border border-[#007979]/20 bg-[#FFF0E4] p-4 text-sm font-bold leading-7 text-[#007979]">
                      {deck.promise}
                    </div>
                    <div className="flex min-w-[220px] flex-col justify-between rounded-[0.9rem] border border-[#007979]/20 bg-[#FFE0C5] p-4 text-sm leading-7 text-[#2d6e6e]">
                      <p>Clean image-led cards and bold headlines for faster scanning.</p>
                      <span className="mt-3 inline-flex w-fit rounded-full border border-[#007979]/25 bg-white px-3 py-1 text-[11px] font-black uppercase tracking-[0.14em] text-[#007979]">
                        Updated Layout
                      </span>
                    </div>
                  </div>
                  <div className="mt-7 flex flex-wrap gap-3">
                    <Link href={basePath} className="rounded-full bg-[#007979] px-5 py-3 text-sm font-black text-white">Browse all</Link>
                    <Link href="/search" className="rounded-full border border-[#007979]/35 bg-white px-5 py-3 text-sm font-black text-[#007979]">Search posts</Link>
                  </div>
                </div>
              </div>

              <form action={basePath} className="rounded-[1.1rem] border border-[var(--editable-border)] bg-white p-5 shadow-[0_10px_24px_rgba(0,121,121,0.1)] lg:sticky lg:top-24">
                <div className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.2em] text-[#007979]"><Filter className="h-4 w-4" /> Filter</div>
                <select name="category" defaultValue={category} className="mt-4 h-12 w-full rounded-xl border border-[#007979]/25 bg-[#FFF0E4] px-4 text-sm font-bold text-[#007979] outline-none">
                  <option value="all">All categories</option>
                  {CATEGORY_OPTIONS.map((item) => <option key={item.slug} value={item.slug}>{item.name}</option>)}
                </select>
                <button className="mt-3 h-12 w-full rounded-xl bg-[#007979] text-sm font-black text-white">Apply</button>
                <p className="mt-3 text-xs font-bold text-[#2d6e6e]">Showing: {categoryLabel}</p>
              </form>
            </div>
          </section>
        ) : (
          <section className="mx-auto grid max-w-[var(--editable-container)] gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[1.05fr_0.95fr] lg:px-8 lg:py-20">
            <div className="rounded-[1rem] border border-[var(--editable-border)] bg-[var(--archive-surface)] p-7 shadow-[0_10px_24px_rgba(0,0,0,0.08)] sm:p-10">
              <div className="inline-flex items-center gap-2 rounded-full border border-[var(--editable-border)] bg-white px-4 py-2 text-xs font-black uppercase tracking-[0.24em] text-[var(--archive-accent)]"><Icon className="h-4 w-4" /> {label}</div>
              <h1 className="mt-5 max-w-4xl font-display text-4xl font-bold leading-[1.04] tracking-[-0.03em] sm:text-5xl">{voice?.headline || `Browse ${label}`}</h1>
              <p className="mt-6 max-w-2xl text-base leading-8 opacity-70">{voice?.description || SITE_CONFIG.description}</p>
              <div className="mt-6 rounded-[0.9rem] border border-[var(--editable-border)] bg-white/55 p-4 text-sm font-bold leading-7 opacity-75">{deck.promise}</div>
            ...
            <form action={basePath} className="self-end rounded-[1rem] border border-[var(--editable-border)] bg-white p-5 shadow-sm backdrop-blur">
        ...
        <section className="mx-auto max-w-[var(--editable-container)] px-4 pb-16 sm:px-6 lg:px-8">
          {posts.length ? (
            <div className={deck.archiveClass}>
              {posts.map((post, index) => <ArchivePostCard key={post.id || post.slug} post={post} task={task} basePath={basePath} index={index} activeCategory={category} />)}
            </div>
          ) : (
            <div className="rounded-[1rem] border border-dashed border-[var(--editable-border)] bg-white p-10 text-center">
...
function ArchivePostCard({ post, task, basePath, index, activeCategory }: { post: SitePost; task: TaskKey; basePath: string; index: number; activeCategory: string }) {
  ...
  return <ArticleArchiveCard post={post} href={href} index={index} activeCategory={activeCategory} />
}

function ArticleArchiveCard({ post, href, index, activeCategory }: { post: SitePost; href: string; index: number; activeCategory: string }) {
  const image = getImage(post)
  const categoryLabel = getCategory(post, 'Article')
  const categorySlug = normalizeCategory(categoryLabel)
  const matchesFilter = activeCategory === 'all' || activeCategory === categorySlug
  return (
    <Link href={href} className="group mx-auto block w-full max-w-[400px] transition duration-300 hover:-translate-y-1">
      <div className="relative aspect-[4/3] overflow-hidden bg-[#007979]/5">
        ...
        <div className="mb-3">
          <span
            className={`inline-flex rounded-full border px-3 py-1 text-[11px] font-black uppercase tracking-[0.12em] ${
              matchesFilter ? 'border-[#007979] bg-[#007979] text-white' : 'border-[#007979]/35 bg-white text-[#007979]'
            }`}
          >
            {categoryLabel}
          </span>
        </div>
        <h2 className="mx-auto max-w-[88%] line-clamp-3 text-[2rem] font-black uppercase leading-[1.15] tracking-[-0.01em] sm:text-[2.1rem]">
          {post.title}
        </h2>
      </div>
    </Link>
...
function ListingArchiveCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <Link href={href} className="group grid gap-5 rounded-[1rem] border border-[var(--editable-border)] bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:shadow-xl sm:grid-cols-[120px_1fr]">
      <div className="flex h-28 w-28 items-center justify-center overflow-hidden rounded-[0.9rem] bg-[#FFF0E4] ring-1 ring-[var(--editable-border)]">
...
function ClassifiedArchiveCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <Link href={href} className="group overflow-hidden rounded-[1rem] border border-[var(--editable-border)] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
...
function ImageArchiveCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  const image = getImage(post)
  return (
    <Link href={href} className="group mb-5 block break-inside-avoid overflow-hidden rounded-[1rem] border border-[var(--editable-border)] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
      <div className={index % 3 === 0 ? 'aspect-[3/4]' : 'aspect-[4/3]'}>
        <img src={image} alt="" className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
      </div>
      <div className="p-5">
        <div className="inline-flex items-center gap-2 rounded-full bg-[#FFF0E4] px-3 py-1 text-[10px] font-black uppercase tracking-[0.16em]"><ImageIcon className="h-3 w-3" /> Visual</div>
...
function BookmarkArchiveCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  ...
  return (
    <Link href={href} className="group block rounded-[1rem] border border-[var(--editable-border)] bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:bg-[var(--archive-text)] hover:text-[var(--archive-bg)]">
...
function PdfArchiveCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <Link href={href} className="group rounded-[1rem] border border-[var(--editable-border)] bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
      <div className="flex items-start justify-between gap-4">
        <div className="rounded-[0.8rem] bg-[var(--archive-text)] p-5 text-[var(--archive-bg)]"><FileText className="h-8 w-8" /></div>
        <span className="rounded-full bg-[#FFF0E4] px-3 py-1 text-[10px] font-black uppercase tracking-[0.18em]">{category}</span>
      </div>
      <h2 className="mt-8 text-2xl font-black leading-tight tracking-[-0.05em]">{post.title}</h2>
      <p className="mt-4 line-clamp-4 text-sm leading-6 opacity-65">{getSummary(post)}</p>
...
function ProfileArchiveCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <Link href={href} className="group rounded-[1rem] border border-[var(--editable-border)] bg-white p-6 text-center shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
      <div className="mx-auto flex h-28 w-28 items-center justify-center overflow-hidden rounded-full bg-[#FFF0E4] ring-1 ring-[var(--editable-border)]">
