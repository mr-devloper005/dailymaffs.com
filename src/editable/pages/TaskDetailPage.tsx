...
function BackLink({ task }: { task: TaskKey }) {
  ...
  return (
    <Link href={taskConfig?.route || '/'} className="inline-flex items-center gap-2 rounded-full border border-[#007979]/30 bg-[#FFF0E4] px-4 py-2 text-sm font-black text-[#007979]">
...
function ArticleDetail({ post, related, comments }: { post: SitePost; related: SitePost[]; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  ...
  const mediaHeightClass = summaryLength > 180 ? 'max-h-[520px]' : 'max-h-[640px]'
  const layoutColsClass = summaryLength > 220 || titleLength > 95
    ? 'lg:grid-cols-[minmax(0,1fr)_420px]'
    : 'lg:grid-cols-[minmax(0,0.88fr)_440px]'
  const contentMaxWidthClass = summaryLength > 220 ? 'max-w-none' : 'max-w-[920px]'
  return (
    <section className={`mx-auto grid max-w-[var(--editable-container)] gap-6 px-4 py-8 sm:px-6 lg:px-8 lg:py-12 ${layoutColsClass}`}>
      <article className={`min-w-0 overflow-hidden rounded-[1.1rem] border border-[#007979]/20 bg-white shadow-[0_14px_36px_rgba(0,121,121,0.14)] ${contentMaxWidthClass}`}>
        <div className="border-b border-[#007979]/15 bg-[#24B1B1]/10 p-5 sm:p-7">
          <div className="flex flex-wrap items-center gap-3">
            <BackLink task="article" />
            <span className="rounded-full border border-[#007979]/30 bg-white px-3 py-1 text-[11px] font-black uppercase tracking-[0.14em] text-[#007979]">
              {categoryOf(post, 'Article')}
            </span>
          </div>
          <h1 className={`mt-4 font-display font-bold leading-[1.03] tracking-[-0.03em] text-[#007979] ${titleClass}`}>{post.title}</h1>
          {summary ? <p className="mt-4 max-w-3xl text-base leading-8 text-[#2d6e6e]">{summary}</p> : null}
        </div>
        {images[0] ? (
          <div className="border-b border-[#007979]/15 bg-[#FFF0E4] p-4 sm:p-6">
            <img src={images[0]} alt="" className={`w-full rounded-[0.95rem] object-cover ${mediaHeightClass}`} />
          </div>
        ) : null}
  ...
  return (
    <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1fr)_420px]">
        <article className="rounded-[1rem] border border-[var(--editable-border)] bg-white p-6 shadow-[0_12px_28px_rgba(0,0,0,0.1)] sm:p-9">
...
function ClassifiedDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <section className="mx-auto grid max-w-[var(--editable-container)] gap-7 px-4 py-10 sm:px-6 lg:grid-cols-[0.82fr_1.18fr] lg:px-8 lg:py-16">
      <aside className="rounded-[1rem] border border-[var(--editable-border)] bg-[var(--detail-text)] p-7 text-[var(--detail-bg)] shadow-xl lg:sticky lg:top-24 lg:self-start">
      ...
      <article className="rounded-[1rem] border border-[var(--editable-border)] bg-white p-6 shadow-[0_12px_26px_rgba(0,0,0,0.1)] sm:p-9">
  ...
  return (
    <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[0.72fr_1.28fr]">
        <aside className="rounded-[1rem] border border-[var(--editable-border)] bg-white p-7 lg:sticky lg:top-24 lg:self-start">
...
function BookmarkDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <section className="mx-auto grid max-w-[var(--editable-container)] gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-8 lg:py-16">
      <article className="rounded-[1rem] border border-[var(--editable-border)] bg-white p-7 shadow-[0_12px_26px_rgba(0,0,0,0.1)] sm:p-10">
        <BackLink task="sbm" />
    ...
    <section className="mx-auto grid max-w-[var(--editable-container)] gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-8 lg:py-16">
      <article className="rounded-[1rem] border border-[var(--editable-border)] bg-white p-6 shadow-[0_12px_26px_rgba(0,0,0,0.1)] sm:p-9">
        ...
        <div className="mt-8 grid gap-6 sm:grid-cols-[120px_1fr]">
          <div className="flex h-28 w-28 items-center justify-center rounded-[0.95rem] bg-[var(--detail-text)] text-[var(--detail-bg)]"><FileText className="h-12 w-12" /></div>
          <div>
...
function ProfileDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <section className="mx-auto grid max-w-[var(--editable-container)] gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[420px_minmax(0,1fr)] lg:px-8 lg:py-16">
      <aside className="rounded-[1rem] border border-[var(--editable-border)] bg-white p-8 text-center shadow-[0_12px_26px_rgba(0,0,0,0.1)] lg:sticky lg:top-24 lg:self-start">
      ...
      <article className="rounded-[1rem] border border-[var(--editable-border)] bg-white p-7 shadow-sm sm:p-10">
        <BodyContent post={post} />
  ...
  return (
    <div className="mt-8 grid gap-3 sm:grid-cols-2">
      {visible.map(([label, value, Icon]) => (
        <div key={label} className="rounded-[0.9rem] border border-[var(--editable-border)] bg-[var(--detail-bg)] p-4">
  ...
  return (
    <section className="mt-8">
      ...
      <div className={`mt-4 grid gap-3 ${large ? 'sm:grid-cols-2' : 'grid-cols-2 sm:grid-cols-4'}`}>
        {images.slice(0, large ? 4 : 8).map((image, index) => <img key={`${image}-${index}`} src={image} alt="" className="aspect-[4/3] rounded-[0.8rem] object-cover ring-1 ring-[var(--editable-border)]" />)}
      </div>
    </section>
  )
}
...
function RelatedPanel({ task, post, related, compact = false }: { task: TaskKey; post: SitePost; related: SitePost[]; compact?: boolean }) {
  ...
  return (
    <aside className="min-w-0 space-y-5">
      {!compact ? (
        <div className="rounded-[1rem] border border-[#007979]/20 bg-white p-5 shadow-[0_10px_24px_rgba(0,121,121,0.1)]">
          <p className="text-xs font-black uppercase tracking-[0.22em] text-[#007979]/70">About this post</p>
          <div className="mt-4 grid gap-3 text-sm font-bold text-[#2d6e6e]">
      ...
      {related.length ? (
        <div className="rounded-[1rem] border border-[#007979]/20 bg-white p-5 shadow-[0_10px_24px_rgba(0,121,121,0.1)]">
          <div className="flex items-center justify-between gap-3">
            <h2 className="text-lg font-black tracking-[-0.04em] text-[#007979]">More like this</h2>
            <Link href={taskConfig?.route || '/'} className="text-xs font-black uppercase tracking-[0.16em] text-[#2d6e6e]">View all</Link>
          </div>
...
function RelatedCard({ task, post }: { task: TaskKey; post: SitePost }) {
  ...
  return (
    <Link href={buildPostUrl(task, post.slug)} className="group flex gap-3 rounded-xl border border-[#007979]/25 bg-[#FFF0E4] p-3 transition hover:-translate-y-0.5 hover:shadow-lg">
      ...
      <div className="min-w-0">
        <h3 className="line-clamp-3 text-sm font-black leading-tight tracking-[-0.03em] text-[#007979]">{post.title}</h3>
        <p className="mt-2 line-clamp-2 text-xs leading-5 text-[#2d6e6e]">{summaryText(post)}</p>
      </div>
    </Link>
  )
}
...
function EditableComments({ slug, comments }: { slug: string; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  return (
    <section className="mt-10 rounded-[1rem] border border-[#007979]/20 bg-[#FFF0E4] p-5">
      <div className="flex items-center gap-2 text-lg font-black text-[#007979]"><MessageCircle className="h-5 w-5" /> Comments</div>
      <div className="mt-5 grid gap-3">
        {comments.slice(0, 5).map((comment) => (
          <div key={comment.id} className="rounded-xl border border-[#007979]/20 bg-white p-4">
            <p className="text-sm font-black text-[#007979]">{comment.name}</p>
            <p className="mt-2 text-sm leading-6 text-[#2d6e6e]">{comment.comment}</p>
          </div>
        ))}
        {!comments.length ? <p className="text-sm text-[#2d6e6e]">No comments yet for {slug}.</p> : null}
      </div>
    </section>
  )
}
